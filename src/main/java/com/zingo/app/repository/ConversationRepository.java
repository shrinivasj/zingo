...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
  @Query(
      "select c from Conversation c where c.showtimeId = :showtimeId "
          + "and c.id in (select m1.conversationId from ConversationMember m1 where m1.userId = :userA) "
          + "and c.id in (select m2.conversationId from ConversationMember m2 where m2.userId = :userB)")
  Optional<Conversation> findExisting(@Param("showtimeId") Long showtimeId, @Param("userA") Long userA, @Param("userB") Long userB);
